...
import { signIn } from '@/app/dsaidsuifds/login/actions'
...
export default function LoginForm() {
  ...
  // Resetăm token-ul la eroare pentru ca Turnstile să poată fi generat corect din nou
  useEffect(() => {
  ...
  // Redirecționare sigură la succes, prevenind Open Redirect
  useEffect(() => {
    if (signInState.success && signInState.redirectTo) {
      const targetUrl = signInState.redirectTo
      // Validare de securitate: permit doar rute interne carecep cu '/' și nu conțin protocol extern
      if (targetUrl.startsWith('/') && !targetUrl.startsWith('//')) {
        router.push(targetUrl)
      } else {
        router.push('/admin') // Fallback sigur în caz de anomalie
      }
    }
  }, [signInState.success, signInState.redirectTo, router])
  ...
  return (
    <main className="auth-shell">
      ...
      <style jsx global>{`
        ...
        .auth-card { position: relative; z-index: 5; width: 100%; max-width: 26rem; padding: 2.25rem 2rem 2rem; border-radius: 1.5rem; background: rgba(12, 7, 5, 0.75); backdrop-filter: blur(22px) saturate(140%); -webkit-backdrop-filter: blur(22px) saturate(140%); border: 1px solid rgba(255, 179, 71, 0.18); box-shadow: 0 24px 60px rgba(0, 0, 0, 0.55); transform: translateZ(0); }
        ...
        .auth-field input { padding: 0.7rem 0.85rem; border-radius: 0.7rem; border: 1px solid rgba(255, 138, 43, 0.2); background: rgba(20, 10, 6, 0.5); color: #ffe9d6; font-size: 0.95rem; outline: none; transition: border-color 0.2s ease, box-shadow 0.2s ease; }
        .auth-field input:focus { border-color: rgba(255, 179, 71, 0.8); box-shadow: 0 0 0 3px rgba(255, 138, 0.18); }
