      ...
      <div className="flex flex-col gap-6 border-b border-brand-black/10 pb-8 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-display text-3xl font-black uppercase tracking-tight text-brand-black sm:text-4xl">
              {member.firstName} {member.lastName}
            </h1>
            <p className="text-sm text-brand-black/60">{member.role}</p>
          </div>
        </div>
      ...
      <div className="mt-10 grid gap-8 lg:grid-cols-2">
        {Object.entries(grouped).map(([category, skills]) => (
          <section key={category}>
            <h2 className="mb-4 flex items-center gap-3 font-display text-sm font-extrabold uppercase tracking-wide text-brand-black">
              ...
              {skills.map((skill) => (
                <li key={skill.name}>
                  <div className="mb-1.5 flex items-baseline justify-between">
                    ...
                    <span className="font-display text-sm font-bold text-brand-black/50">
                      {skill.level}%
                    </span>
                  </div>
                  <div className="h-2.5 w-full overflow-hidden rounded-full bg-brand-black/10">
                    <div
