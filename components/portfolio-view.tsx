...
export function PortfolioView() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
      {!activeMember && (
        <p className="mb-6 rounded-md border border-brand-orange/40 bg-brand-orange/10 px-4 py-2 text-sm text-brand-black/70">
      ...
      <div className="flex flex-col gap-6 border-b border-brand-black/10 pb-8 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="font-display text-4xl font-black uppercase tracking-tight text-brand-black sm:text-5xl">
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {member.portfolioWorks.map((work, i) => (
          <article
            key={work.id}
            className="group flex flex-col overflow-hidden rounded-lg border-2 border-brand-black/10 bg-brand-cream transition-colors hover:border-brand-orange"
          >
            ...
            <div className="flex flex-1 flex-col p-5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-brand-black/50">{work.year}</span>
              </div>
              <h2 className="mt-2 font-display text-xl font-extrabold text-brand-black">
                {work.title}
              </h2>
              <p className="mt-2 text-pretty text-sm leading-relaxed text-brand-black/65">
                {work.description}
              </p>
            </div>
          </article>
